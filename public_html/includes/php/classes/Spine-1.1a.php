...
class Spine {
	...
	private $spine_limit = 17;
	private $spine_chunk_breaks = array(0, 8, 13);
		...
		if (!empty($posts)) {
			foreach ($posts as $i => $post) {
				if (!empty($post['width'])) {
					...
					//$url = $post['source'] . 'image.php?imagename=' . $post['imagename'] . '&width=' . $post['width'] . '&height=' . $post['height'];
				...
				<ul class="posts">
					...
					foreach ($posts as $i => $post) {
						...
						<li class="<?= !empty($post['is_voted']) ? ' voted' : '' ?>" data-posting_id="<?= $post['posting_id'] ?>" data-vote_period_id="<?= $post['vote_period_id'] ?>">
							...
							<div class="image">
								<img src="<?= $post['image_url'] ?>&amp;width=420" />
								<?
								/*
	...
	public function output_images($images, $class, $url) {
		...
		if (empty($images)) {
			...
			foreach ($images as $i => $image) {
				$mod = $i % $this->spine_limit;
				
				//if ($mod == 0 || $mod == 8 || $mod == 13) {
				if (in_array($mod, $this->spine_chunk_breaks)) {
					if ($i > 0) {
						?></ul><?
					}
			...
			if (!$is_partial) {
				...
				<script type="text/javascript">
					...
					$(document).on('click', '.spine a[rel~="modal"]', function(event) {
						if (!$('body').hasClass('mobile')) {
							...
							$modal_content.load(href, function() {
								...
								$modal.show(0, function() {
									var margin_top = $(document).scrollTop() + 50;
									var margin_left = -1 * Math.floor($modal_content.outerWidth() / 2);
									$modal_content.css({

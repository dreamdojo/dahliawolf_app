...
class db extends PDO { 
	...
	public function __construct( $dsn='', $username='', $password='', $driver_options=array() ) { 
		...
		if ( ! defined( 'PDO::ATTR_DRIVER_NAME' ) ) {
			$this->error = "databasePDO requires PDO extension.";
			exit( $this->error );
		}
		
		// Check if PHP version is compatible.
		if( version_compare( PHP_VERSION, $this->php_ver, '<') ) {
			$this->error = "databasePDO requires PHP version $this->php_ver and higher.";
			exit( $this->error );
		}
	...
	public function get( $table=null, $where=array(), $fields='*', $type='all' ) {
		...
		if( $type == 'var' ) {
			...
			$this->result=$stmt->fetchAll(); // Fetch a single row.
	...
	public function debug() {
		...
		echo '<pre>';
		echo 'databasePDO - PHP Mysql Database Helper Class - v' . $this->databasePDO_ver . "\r\n\r\n";

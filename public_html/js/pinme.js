	...
	if(!document.scriptolutionRunning)
	{
		...
		for(var j=0;j<imgs.length;j++)
		{
			(function(img){
				...
				{
					window.open(popupUrl+'?url='+escape(img.src)+'&sourceurl='+escape(location.href)+'&domain='+escape(document.domain)+'&title='+document.title,"scriptolution_"+(new Date).getTime(),"status=no,resizable=no,scrollbars=yes,personalbar=no,directories=no,location=no,toolbar=no,menubar=no,width=632,height=270,left=0,top=0");
					close();
					return false;
				}

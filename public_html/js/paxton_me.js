(function() {
    ...
    fileref.setAttribute("src", 'http://www.dahliawolf.com/js/jquery.js');
    document.getElementsByTagName("head")[0].appendChild(fileref);

	var base_url = 'http://www.dahliawolf.com/';
    ...
    if (window.location.href.match(/^(http|https):\/\/(www\.dahliawolf\.com)/))
	{
        alert('The Post Button has been successfully installed. You can now post images from your favorite websites to Dahliawolf!');
        return false;
    }
	if(!document.scriptolutionRunning)
	{
		...
		if(imgs.length == 0)
		{
			alert('Unfortunately we were unable to locate any images big enough to be posted.');
		...
		var s = '#scriptolutionOverlay {position: fixed; z-index: 9999; top: 0; right: 0; bottom: 0; left: 0; background-color: #ffffff; opacity: .95; overflow: scroll}';
		...
		s+= '#scriptolutionContainer {position: absolute; z-index: 9999; top: '+ vscroll +'px; left: '+ hscroll +'px; right: 0;}';
		...
		html+= '<div id="scriptolutionLogo"><a target="_blank" href="' + base_url +'"><img src="' + base_url +'images/logo_no_beta.png" alt="" /></a></div>';
		html+= '<div id="scriptolutionBorder"></div>';
		...
		for(var j=0;j<imgs.length;j++)
		{
			(function(img){
				...
				{
                    var url = 'http://dev.dahliawolf.com/api/1-0/boosted.json?function=load&use_hmac_check=0&domain='+document.domain+'&image_url='+ encodeURI(img.src);
                    //xmlhttp = new XMLHttpRequest();
                    //xmlhttp.open("GET", url, true);
                    //xmlhttp.send();
					//var blop = window.open(popupUrl+'?url='+escape(img.src)+'&sourceurl='+escape(location.href)+'&domain='+escape(document.domain)+'&title='+document.title,"scriptolution_"+(new Date).getTime(),"status=no,resizable=no,scrollbars=yes,personalbar=no,directories=no,location=no,toolbar=no,menubar=no,width=632,height=270,left=0,top=0");
                    var blop = window.open(url, 'added', "status=no,resizable=no,scrollbars=yes,personalbar=no,directories=no,location=no,toolbar=no,menubar=no,width=632,height=270,left=0,top=0");
                    setTimeout(function() {
                        blop.close();

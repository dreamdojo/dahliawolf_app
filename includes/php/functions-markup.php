...
function output_order_details($order) {
	if (empty($order['products'])) {
        ...
        output_order_table($order);
        output_order_footer($order);
	}
}

function output_order_table($order, $is_return = false) {
	?>
	<table class="cart">
        <thead>
            <tr>
                ...
                if ($is_return) {
                    ...
                    <th scope="col" class="quantity no-wrap">Exchange Size</th>
                	<?
				}
                ?>
            </tr>
				...
				<tr>
	                ...
	                if ($is_return) {
	                	...
	                	<td scope="col" class="quantity">
							<input type="number" min="0" max="<?= $product['product_quantity'] ?>" step="1" value="0" name="id_order_detail[<?= $product['id_order_detail'] ?>]">
						</td>
                        <td scope="col" class="quantity">
							<? 
							if (!empty($product['combinations'])) {
								?>
                                <select name="product_attribute_id[<?= $product['id_order_detail'] ?>]" value="1" />
                                    <?
                                    foreach ($product['combinations'] as $j => $combination) {
										$selected = ($product['product_attribute_id'] == $combination['id_product_attribute']) ? ' selected="selected"' : '';
                                        ?>
                                        <option value="<?= $combination['id_product_attribute'] ?>"<?= $selected ?>><?= str_replace('Size: ', '', $combination['attribute_names']) ?></option>
                                        <?
                                        }
                                    ?>
                                </select>
...
function output_order_footer($order) {
	?>

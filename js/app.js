...
function changesizes(container){
    $ci=2;
    $(container).find('div.pin').each(function(){
    if($ci==8){
        $ci=2;
    }
    var $this = $(this),
    number =  $this.find('.sysPinImg').attr("id");
    var num=number.split("-");
    number2 = parseInt(num[1]);
    $("#"+number).addClass('width'+$ci)
      $ci++;
    })
}
